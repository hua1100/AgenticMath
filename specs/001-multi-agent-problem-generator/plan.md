...
### ✅ Phase 1: Design (COMPLETED - 2025-11-06)

**已完成任務**：
1. ✅ 更新 `spec.md` - 加入照片上傳需求 (User Story 0)
2. ✅ 更新 `data-model.md` - 加入 UploadedImage 實體
3. ✅ 創建 `contracts/image-extraction-agent.md` - OCR 合約
4. ✅ 更新所有 Agent contracts - 改為繁體中文（決定不需要中文化）
5. ✅ 設計 OCR → Agent 流程的狀態機 - 見 `workflow-state-machine.md`
6. ✅ 創建 quickstart guide - 含照片上傳範例 - 見 `quickstart.md`
7. ✅ 定義配置 schema - 加入 OCR 設定 - 見 `config-schema.md`

**設計文檔總覽**：
- `spec.md` - 功能規格（含 User Story 0: Photo Upload & OCR）
- `data-model.md` - 資料模型（含 UploadedImage 實體、OCR 欄位）
- `contracts/image-extraction-agent.md` - OCR Agent 合約
- `contracts/rephrase-agent.md` - Rephrase Agent 合約
- `contracts/review-agent.md` - Review Agent 合約
- `contracts/revise-agent.md` - Revise Agent 合約
- `contracts/solver-agent.md` - Solver Agent 合約
- `workflow-state-machine.md` - 完整工作流程狀態機
- `quickstart.md` - 快速開始指南（含照片上傳範例）
- `config-schema.md` - 配置 schema 定義（含 OCR 設定）
- `tasks.md` - 實作任務分解（21 個任務，預估 84 小時）

### ⏭️ Phase 2: Implementation (準備開始)

**實作任務已規劃**（見 `tasks.md`）：
- Phase 0: 專案設置（2 任務，5 小時）
- Phase 1: User Story 0 - Photo Upload & OCR（6 任務，29 小時）
- Phase 2: User Story 1 - Problem Rephrase + Quality Control（6 任務，32 小時）
- Phase 3: User Story 2 - Solution Generation（2 任務，10 小時）
- Phase 4: User Story 3 - Configuration & CLI（2 任務，8 小時）
- Phase 5: Testing & Quality Assurance（3 ongoing 任務）

**總計**：21 任務，預估 84 小時（約 6 週）

**下一步**：開始 Phase 0 - Task 0.1: Initialize Project Structure
