...
2. **Qwen (通義千問)** (開源 + API)
3. **GLM-4** (智譜 AI, API)
4. **OpenAI GPT-4** (API, 參考)
5. **Anthropic Claude** (API, 參考)

### 中文數學 LLM 快速比較

| LLM | 開源 | 中文能力 | 數學能力 | 成本 (估算) | 推薦度 |
|-----|------|---------|---------|------------|--------|
| **DeepSeek-V3** | ❌ API | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ¥0.001/1K tokens | 🥇 **首選** |
| **Qwen-Max** | ⚠️ 混合 | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ¥0.02/1K tokens | 🥈 **備選** |
| **GLM-4** | ❌ API | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ¥0.05/1K tokens | ⭐ **可考慮** |
| **GPT-4** | ❌ API | ⭐⭐⭐ | ⭐⭐⭐⭐⭐ | $0.03/1K tokens | ⚠️ **昂貴** |
...
**備註**：LLM 層面較難完全開源（需要巨大 GPU），API 服務是實際選擇。
...
| DeepSeek API 限流 | 中 | 實現重試機制，準備 Qwen 備選 |
| 開源方案功能不足 | 低 | 文檔中保留商業方案備選評估 |

---

**研究結論**：
本研究推薦使用 **CrewAI + PaddleOCR + DeepSeek-V3** 作為 MVP 技術棧，符合所有憲章原則，同時平衡了功能、成本和開發速度。
